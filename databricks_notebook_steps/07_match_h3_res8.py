...
# MAGIC agreement is weighted more heavily.
...
H3_RES8_MATCH_OUTPUT_TABLE = "TODO_CATALOG.TODO_SCHEMA.dc_byte_client_matches_h3_res8"

# COMMAND ----------

if (
    DC_BYTE_H3_RES8_TABLE.startswith("TODO_")
    or CLIENT_H3_RES8_TABLE.startswith("TODO_")
    or H3_RES8_MATCH_OUTPUT_TABLE.startswith("TODO_")
):
...
dc = spark.table(DC_BYTE_H3_RES8_TABLE).filter(F.col("h3_res8").isNotNull())
client = spark.table(CLIENT_H3_RES8_TABLE).filter(F.col("h3_res8").isNotNull())

candidate_pairs = (
    dc.alias("dc")
    .join(client.alias("cl"), F.col("dc.h3_res8") == F.col("cl.h3_res8"), "inner")
    .select(
        F.col("dc.h3_res8").alias("h3_res8"),
...
scored_pairs = (
    ...
    .withColumn(
        ...
        F.col("country_match")
        | (F.length("dc_country_match_key") == 0)
        | (F.length("client_country_match_key") == 0),
    )
    .withColumn(
        "combined_score",
        F.round(
            F.least(
                F.lit(100.0),
                (F.col("address_score") * F.lit(0.44))
                + (F.col("account_name_score") * F.lit(0.26))
                + (F.col("location_name_score") * F.lit(0.20))
                + (F.col("city_score") * F.lit(0.10))
                + F.when(F.col("country_match"), F.lit(4.0)).otherwise(F.lit(0.0)),
            ),
            2,
        ),
    )
...
matched_pairs = scored_pairs.withColumn(
    "match_rule",
    F.when(
        F.col("country_compatible")
        & (F.col("address_score") >= 90)
        & (F.col("city_score") >= 70)
        & ((F.col("account_name_score") >= 60) | (F.col("location_name_score") >= 72)),
        F.lit("res8 address plus account/location/city"),
    )
    .when(
        F.col("country_compatible")
        & (F.col("address_score") >= 96)
        & (F.col("city_score") >= 85),
        F.lit("res8 very strong address plus city"),
    )
    .when(
        F.col("country_compatible")
        & (F.col("address_score") >= 84)
        & (F.col("account_name_score") >= 88)
        & (F.col("location_name_score") >= 75)
        & (F.col("city_score") >= 82),
        F.lit("res8 address plus strong account/name/city"),
    )
    .when(
        F.col("country_compatible")
        & (F.col("address_score") >= 78)
        & (F.col("account_name_score") >= 94)
        & (F.col("location_name_score") >= 82)
        & (F.col("city_score") >= 85),
        F.lit("res8 strong account/name with address validation"),
    ),
